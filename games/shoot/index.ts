...
export default class ShootGame implements types.Game<ShootGameApi> {
  ...
  private collides(): boolean {
    if (this.ballX < TARGET_X) {
      ...
      if (this.ballY > this.targetY + TARGET_H) return l2dist(this.ballX, this.ballY, TARGET_X, this.targetY + TARGET_H) <= BALL_R;
      return this.ballX >= (TARGET_X - BALL_R);
    } if (this.ballX > TARGET_X + TARGET_W) {
      if (this.ballY < this.targetY) return l2dist(this.ballX, this.ballY, TARGET_X + TARGET_W, this.targetY) <= BALL_R;
      if (this.ballY > this.targetY + TARGET_H) return l2dist(this.ballX, this.ballY, TARGET_X + TARGET_W, this.targetY + TARGET_H) <= BALL_R;
      return this.ballX <= (TARGET_X + TARGET_W + BALL_R);
    }
